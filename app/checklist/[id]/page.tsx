...
import { 
  ...
  Info, MapPin, XCircle, Plus, Trash2, UploadCloud
} from 'lucide-react';
...
const SUCCESS_SOUND_URL = "https://mdboxppkyxfxwtuwcpcz.supabase.co/storage/v1/object/public/checklist-photos/public/successed-295058.mp3";
...
// MUDANÇA: O estado agora guarda um array de fotos
...
export default function ChecklistPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    for (const q of MOCKED_QUESTIONS) {
        ...
        if (answers[q.id]?.answer === 'nao' && (!answers[q.id]?.photos || answers[q.id].photos.length === 0)) {
    ...
    // MUDANÇA: Upload de múltiplas fotos em paralelo
    for (const questionId in answers) {
      ...
      if (currentAnswer.answer === 'nao' && currentAnswer.photos && currentAnswer.photos.length > 0) {
        ...
        // Cria uma 'promessa' de upload para cada foto
        ...
        for (const result of uploadResults) {
            if (result.error) {
                alert(`Erro no upload de uma das fotos. Tente novamente.`);
  ...
  return (
    <main className="min-h-screen w-full bg-gray-100 font-sans">
      ...
      <div className="bg-purple-800 p-4 shadow-md flex justify-between items-center text-white sticky top-0 z-10">
        ...
        <h1 className="font-bold text-lg truncate px-2">{checklist?.name}</h1>
      ...
      <div className="p-4 sm:p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          {MOCKED_QUESTIONS.map((question, index) => (
            <motion.div key={question.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
              ...
              {/* MUDANÇA: Galeria de Imagens e Botão de Adicionar */}
              <AnimatePresence>
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-4 gap-4">
                      ...
                      {answers[question.id]?.photos?.map((photo, photoIndex) => (
                        <div key={photoIndex} className="relative aspect-square group">
                          <img src={URL.createObjectURL(photo)} alt={`Preview ${photoIndex + 1}`} className="w-full h-full object-cover rounded-md" />
                          <button type="button" onClick={() => handleRemovePhoto(question.id, photoIndex)} className="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Trash2 className="h-4 w-4" />
                          </button>
                        </div>
                      ))}
                      {/* Botão de Adicionar Foto */}
                      <button type="button" onClick={() => fileInputRef.current[question.id]?.click()} className="flex flex-col items-center justify-center aspect-square border-2 border-dashed border-gray-300 rounded-md text-gray-500 hover:bg-gray-50 hover:border-purple-500 transition-colors">
                        <UploadCloud className="h-8 w-8" />
                        <span className="text-sm mt-1">Adicionar foto</span>
                      </button>
                    </div>
